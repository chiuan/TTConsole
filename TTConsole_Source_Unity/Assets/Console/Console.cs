/*
 * @Author: chiuan wei 
 * @Date: 2015-03-03 16:55:05 
 * @Last Modified by: chiuan wei
 * @Last Modified time: 2017-05-14 00:08:30
 */
namespace TinyTeam.Debuger
{
    ...
    public class Console : MonoBehaviour
    {
        ...
        List<string> _outputToUnityMsgType = new List<string>();

        Dictionary<string, string> _btnTable = new Dictionary<string, string>();
        ...
        void Init()
        {
            ...
            // init the output to unity msg
            string outmsg = PlayerPrefs.GetString("TTDEBUG_2U_MSG", "");
            string[] outmsgsplits = outmsg.Split(' ');
            foreach (var s in outmsgsplits)
            {
                if (_outputToUnityMsgType.Contains(s) == false)
                {
                    _outputToUnityMsgType.Add(s);
                }
            }
        ...
        RectTransform btnParent = null;
        void InitButtons(GameObject windows)
        {
            ...
            btnParent.anchorMax = Vector2.one;
            btnParent.pivot = new Vector2(0.5f, 0.5f);
            btnParent.anchoredPosition = Vector2.zero;
        }

        void UpdateButtons()
        {
            while (btnParent.childCount > 0)
                DestroyImmediate(btnParent.GetChild(0).gameObject);

            var enumerator = _btnTable.GetEnumerator();
            int rowCount = (int)(btnParent.rect.width) / 100;
            int count = 0;
            while (enumerator.MoveNext())
            {
                ...
                btn_text.rectTransform.pivot = Vector2.zero;
                btn_text.rectTransform.anchoredPosition = Vector2.zero;
                btn_text.rectTransform.sizeDelta = new Vector2(100, 30);
                btn_text.font = Resources.GetBuiltinResource(typeof(Font), "Arial.ttf") as Font;
                btn_text.color = Color.white;
                btn_text.alignment = TextAnchor.MiddleCenter;
                btn_text.supportRichText = false;

                UnityEngine.UI.Button btn = cmd_btn.AddComponent<UnityEngine.UI.Button>();
                string cmd = enumerator.Current.Value;
                btn.onClick.AddListener(() => { ProcessCmd(cmd); });
                count++;
            }
        }
        ...
        object CMDSetOutputToUnityMsgType(string[] args)
        {
            ...
            if (args == null || args.Length == 1)
            {
                _outputToUnityMsgType.Clear();
                PlayerPrefs.SetString("TTDEBUG_2U_MSG", "");
                return "清理所有设置输出到Unity控制台的消息类型";
            }
            else
            {
                for (int i = 1; i < args.Length; i++)
                {
                    string msg = args[i];
                    msg = msg.ToLower();
                    if (_outputToUnityMsgType.Contains(msg) == false)
                    {
                        _outputToUnityMsgType.Add(msg);
                        ret += msg + " ";
                    }
                }

                string cached = "";
                _outputToUnityMsgType.ForEach((str) =>
                {
                    cached += (str + " ");
                });
                PlayerPrefs.SetString("TTDEBUG_2U_MSG", cached);
            }

            return "设置 " + ret + "消息输出到Unity控制台";
        }
        ...
        public static void RegisterButton(string btnName, string cmd)
        {
            if (Console.Instance != null)
            {
                Console.Instance._btnTable[btnName] = cmd;
            }
        }
        ...
        public static void UnRegisterButton(string btnName)
        {
            if (Console.Instance != null)
            {
                Console.Instance._btnTable.Remove(btnName);
            }
        }
        #endregion
